...
### `The cluster element was not found in the JSON`（Unflatten From JSON で発生）

**意味**：LV側のクラスタに書いてある名前が、返ってきたJSONに見つからない。

方向を間違えないこと:
- JSONにあってクラスタに**無い**キー（`turns` / `count`）→ **無視される。エラーにならない**
- クラスタに**あって**JSONに**無い**名前 → **このエラー**

`/api/lv/multi` のトップレベルに存在するキーは**次の5つだけ**。

```
ok / turns / transcript / count / error
```

`reply` / `agentId` / `agentName` / `model` は**トップレベルには無い**（`turns` 配列の中にある）。

疑う順番:
1. **古い要素の消し忘れ**（最有力）。単体AI版から複製したため `reply` などが残っていないか。
   受信クラスタの中身は `ok` / `transcript` / `error` の**3つだけ**にする。
2. **URLの変え忘れ**。`/api/lv/chat` のままだと `transcript` が返らないため同じエラーになる。
3. **ラベルではなくキャプションを変えた**。JSONのキーになるのは**ラベル**。
   右クリック → `Visible Items > Label` で、表示されている文字を確認する。
4. **ラベルの付け忘れ**。既定の `String` のままだとJSONに無い名前として扱われる。
5. **大文字小文字**。クラスタ内のラベルは小文字の `transcript`
   （フロントパネルの表示器名が `Transcript` でも構わないが、**クラスタのラベルは別物**）。

切り分け方: まず **`ok` 1個だけ**のクラスタにして実行する。
通れば残りの名前が原因、それでも出るならURLを疑う。
